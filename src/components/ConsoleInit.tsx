    ...
    (async () => {
      try {
        ...
        for (const { text, font, color } of bannerConfigs) {
          ...
          setTimeout(() => customConsole.log(ascii, color as any), delay);
          delay += 500;
        }

        // Hidden recruitment messages
        setTimeout(() => {
          customConsole.log("═══════════════════════════════════════════════", "purple");
          customConsole.log("👁  You found the console. Curious minds are welcome.", "cyan");
          customConsole.log("🔓  Try typing 'hack', 'matrix', 'midnight', 'jinwoo', or the Konami code.", "green");
          customConsole.log("📡  Want to connect? → /contact", "pink");
          customConsole.log("═══════════════════════════════════════════════", "purple");
        }, delay + 500);
      } catch {
        // Silently fail if fonts can't load
        customConsole.log("=== ONSLAUGHT2342 ===", "green");
        customConsole.log("Security Researcher | Ethical Hacker", "cyan");
      }
    })();
  }, [enabled]);
