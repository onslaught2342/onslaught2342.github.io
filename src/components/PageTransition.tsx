...
const TerminalLoader = ({ path }: { path: string }) => {
	...
	return (
		...
		>
			<div className="space-y-2 text-center font-mono text-xs text-primary sm:text-sm">
				<p className="text-muted-foreground/60">$ cd {path || "/"}</p>
				<p>
					[{bar}] {Math.floor(progress * 100)}%
				</p>
				<p className="animate-glow-pulse text-[10px] text-muted-foreground/40">
					Loading modules...
				</p>
			</div>
		</motion.div>
