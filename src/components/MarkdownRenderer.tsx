...
function parseMarkdown(raw: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('md-codeblock')) { out.push(line); continue; }
    ...
    if (headingMatch) {
      ...
      out.push(`<h${level} id="${id}" class="md-h${level}">${inlineFormat(text)}</h${level}>`);
      continue;
    }

    if (line.startsWith('> ')) {
      if (inList) { out.push('</ul>'); inList = false; }
      out.push(`<blockquote class="md-blockquote">${inlineFormat(line.slice(2))}</blockquote>`);
      continue;
    }
